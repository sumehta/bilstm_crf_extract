...
class Preprocessor:
    def __init__(self, config_dir, save_config_dir=None, verbose=True):
        ...
        self.tags, self.tags_dict = self.__load_list_file(FILE_TAGS, offset=0, verbose=verbose)
        if save_config_dir:
            self.__save_config(save_config_dir)

        self.PAD_IDX = 0
        self.TAG_PAD_IDX = len(self.tags)+2 # last two tags are reserved for start and stop tags
    ...
    def load_dataset(self, corpus_dir, val_split, test_split, max_seq_len):
        ...
        xs, ys = self.__build_corpus(corpus_dir, FILE_DATASET, max_seq_len)
        xs_test, ys_test = self.__build_corpus(corpus_dir, TEST_DATASET, max_seq_len)
        # else:
        ...
        indices = np.cumsum([0, train_count, val_count])
        datasets = [(xs[s:e], ys[s:e]) for s, e in zip(indices[:-1], indices[1:])]
        datasets.append((xs_test, ys_test))
    ...
    def tags_to_vector(self, tags, max_seq_len=0):
        ...
        return vec + [self.TAG_PAD_IDX] * (max_seq_len - len(vec))

    def __build_corpus(self, corpus_dir, dataset_path, max_seq_len):
        file_path = join(corpus_dir, dataset_path)
        xs, ys = [], []
